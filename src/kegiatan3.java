...
class Student extends User {
    ...
    public void displayInfo() {
        ...
        System.out.println("Program Studi: " + studyProgram);
    }
    ...
    public void showBorrowedBooks(HashMap<String, String[]> bookList) {
        ...
        for (String bookID : borrowedBooks) {
            String[] book = bookList.get(bookID);
            if (book != null) {
                System.out.printf("ID: %-5s || Judul: %-30s || Penulis: %-20s || Durasi Peminjaman: %-5s hari\n", bookID, book[0], book[1], book[3]);
            }
        }
    }
    ...
    public void borrowBook(String bookID, HashMap<String, String[]> bookList) {
        ...
        if (book != null) {
            ...
            if (stock > 0) {
                ...
                // Input waktu peminjaman
                System.out.print("Masukkan durasi peminjaman (dalam hari): ");
                int duration = kegiatan3.scanner.nextInt();
                // Tambahkan waktu peminjaman ke buku
    ...
    public void returnBooks() {
        ...
        System.out.println("Buku yang dipinjam:");
        showBorrowedBooks(kegiatan3.bookList);
        System.out.print("Masukkan ID Buku yang ingin dikembalikan: ");
        String bookID = scanner.next();
        if (borrowedBooks.contains(bookID)) {
            String[] book = kegiatan3.bookList.get(bookID);
            if (book != null) {
                int stock = Integer.parseInt(book[2]);
                stock++;
                book[2] = String.valueOf(stock);
...
class Admin extends User {
    private static final String adminUsername = "nizar";
    private static final String password = "nizar321";
    public ArrayList<String[]> userStudentList;

    public Admin() {
        userStudentList = new ArrayList<>();
        // Menambahkan mahasiswa permanen
        userStudentList.add(new String[]{"202310370311186", "ahmad Nizar", "teknik", "informatika"});
    }
    ...
    public void inputBook() {
        ...
        switch (choice) {
            case 1:
                kegiatan3.bookList.put(bookID, new String[]{title, author, String.valueOf(stock), "Story Book"});
                break;
            case 2:
                kegiatan3.bookList.put(bookID, new String[]{title, author, String.valueOf(stock), "History Book"});
                break;
            case 3:
                kegiatan3.bookList.put(bookID, new String[]{title, author, String.valueOf(stock), "Text Book"});
                break;
            default:
...
public class kegiatan3 {
    ...
    public kegiatan3() {
        bookList.put("1234", new String[]{"Kalkulus", "Soyan Afrinato", "5", "Text Book"});
        bookList.put("5678", new String[]{"MTK Diskrit", "Soyan Afrinsto", "3", "Text Book"});
        for (String bookID : bookList.keySet()) {
